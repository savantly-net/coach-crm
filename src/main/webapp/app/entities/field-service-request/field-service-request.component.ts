...
export class FieldServiceRequestComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.fieldServiceRequestService
                ...
                .subscribe(
                    (res: HttpResponse<IFieldServiceRequest[]>) => this.paginateFieldServiceRequests(res.body, res.headers),
    ...
    registerChangeInFieldServiceRequests() {
        this.eventSubscriber = this.eventManager.subscribe('fieldServiceRequestListModification', response => this.reset());
    }
    ...
    protected paginateFieldServiceRequests(data: IFieldServiceRequest[], headers: HttpHeaders) {
        this.links = this.parseLinks.parse(headers.get('link'));
        this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
        for (let i = 0; i < data.length; i++) {
            this.fieldServiceRequests.push(data[i]);
        }
    }
